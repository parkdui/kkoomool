...
export async function listPublicDreams(n = 80) {
  const ref = collectionGroup(getDb(), "records");
  const q = query(ref, where("isPublic", "==", true), orderBy("createdAt", "desc"), limit(n));
  const snap = await getDocs(q);
  return snap.docs.map((d) => {
    const data = d.data();
    const pathUserId = d.ref?.parent?.parent?.id || "";
    return {
      id: d.id,
      ...data,
